...
export class CountryService {
  ...
  cacheKey: string = "countriesKey";
   ...
   async findAll(): Promise<CountryEntity[]> {
       ...
       if(!cached){
        const countries: CountryEntity[] = await this.countryRepository.find({ relations: ["culinaryCultures"] });
   ...
   async findOne(id: string): Promise<CountryEntity> {
       const country: CountryEntity = await this.countryRepository.findOne({ where: { id }, relations: ["culinaryCultures"] });
       if (!country)
